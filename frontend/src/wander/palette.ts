// palette.ts — palette adapter for the Pixi render path.
//
// The SVG engine colours itself purely through CSS: `wander.css` defines the
// time-of-day moods as custom properties on `.wander[data-palette="…"]`, and
// the procedural art references them via `var(--…)`. The Pixi path can't lean
// on CSS class resolution, so this module reads the *computed* values of those
// same custom properties off the live `.wander` root and exposes them as Pixi
// numeric tints. This keeps the WebGL scene in lock-step with whatever palette
// the DOM is using (and with the HTML content panels, which still get their
// colours from the CSS vars).
 ...
 * that can't be parsed (e.g. an unsupported `color-mix()` left unresolved by an
 * old engine) falls back to the sunset default for that slot.
 */
